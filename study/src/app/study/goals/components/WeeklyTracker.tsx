...
import { addGoalToDay, toggleDayGoalCompletion, removeGoalFromDay, Goal, WeeklyGoals } from '@/lib/redux/features/goals/goalsSlice';
...
const WeeklyTracker: React.FC = () => {
  ...
  const weeklyGoals = useAppSelector(state => state.goals.weeklyGoals);
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, day: DayName) => {
    e.preventDefault();
    e.currentTarget.classList.remove('bg-gray-50');
    try {
      const goalData: Goal = JSON.parse(e.dataTransfer.getData('goal'));
      dispatch(addGoalToDay({ goal: goalData, day }));
    } catch (error) {
    ...
    return (
      <div key={day} className="flex flex-col h-full">
        <div className={`text-center py-3 mb-2 ${isToday ? 'border-b-2 border-green-600' : ''}`}>
          <h3 className="text-[#4a3728] font-semibold text-sm md:text-base">{day}</h3>
            ...
            dayGoals.map((goal) => (
              ...
              >
                <div className="flex items-start gap-2">
                  <div className="w-2 h-2 mt-1 rounded-full flex-shrink-0" style={{ backgroundColor: goal.color }} />
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-[#3b2a1e] text-sm break-words">{goal.title}</h4>
                  </div>
                  <button
                    onClick={() => dispatch(toggleDayGoalCompletion({ day, goalId: goal.id }))}
                ...
                <button
                  onClick={() => dispatch(removeGoalFromDay({ day, goalId: goal.id }))}
      ...
      <div className="md:hidden">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            <p className="text-xs text-[#6b5847] mb-1">{currentDayIndex + 1} of 7</p>
            <div className="flex gap-1">
              {days.map((_, index) => (
                <div
                  key={index}
                  className={`w-2 h-2 rounded-full transition-all ${
                    index === currentDayIndex ? 'bg-[#4a3728] w-6' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>
